...
static double calcuBeta(const Mat& _img) {
	...
	else beta = 1.0 / (2 * totalDiff / (8 * _img.cols*_img.rows - 6 * _img.cols - 6 * _img.rows + 4));   //经过推导得出，详细推导步骤可以具体说 
	return beta;
}
...
static void initGMMs(const Mat& img, const Mat& mask, GMM& bgdGMM, GMM& fgdGMM) {
	...
	kmeans(_bgdSamples, GMM::K, bgdLabels,
		TermCriteria(cv::TermCriteria::MAX_ITER + cv::TermCriteria::EPS, kmeansItCount, 0.0), 0, cv::KMEANS_PP_CENTERS);
	Mat _fgdSamples((int)fgdSamples.size(), 3, CV_32FC1, &fgdSamples[0][0]);
	kmeans(_fgdSamples, GMM::K, fgdLabels,
		TermCriteria(cv::TermCriteria::COUNT + cv::TermCriteria::EPS, kmeansItCount, 0.0), 0, cv::KMEANS_RANDOM_CENTERS);
